...
class Film:
    ...
    def size(self):
        if not self._size:
            self._size = fops.size(self.source_path)
        return self._size
    ...
    def has_valid_ext(self):
        return fops.file.has_valid_ext(self.source_path) if self.is_file else False
    ...
    def should_ignore(self):
        if fops.file.contains_ignored_strings(self.original_filename):
            self.ignore_reason = '(ignored string)'

        if self.is_file and not self.has_valid_ext:
            self.ignore_reason = '(not a valid file extension)'

        if self.is_dir and len(fops.dir.get_valid_files(self.source_path)) == 0:
        ...
        if self.year is None and (config.force_lookup is False or config.tmdb.enabled is False):
            self.ignore_reason = '(unknown year)'

        if self.size < config.min_filesize * 1024 * 1024 and self.is_video_file:
    ...
    def duplicates(self):
        ...
        duplicates = list(ifilter(lambda d: 
                ...
                and ((
                    ...
                    # one valid file.
                    and len(fops.dir.get_valid_files(d.source_path)) > 0)
        ...
        if len(duplicates) > 0:
            ...
            for d in duplicates:
                console.warn("  '{}' is {} ({})".format(
                    ...
                    formatter.pretty_size(fops.size(d.source_path))))
