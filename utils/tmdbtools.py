...
def search(title, year=None, recur=True):

    unmutilatedTitle = title

    # Disable logging
    o.disableLogging()

    if title is None: return
    o.debug("Searching {} {}".format(title, year))

    # Search
    s = tmdb.Search()
    response = s.movie(query=title, primary_release_year=year, include_adult='true')
    results = s.results
    ...
    if len(results) == 0 and stripArticles.match(title):
        ...
        return search(re.sub(stripArticles, '', title), year)
    ...
    # ... recursively remove the last word of the title
        ...
        for i, word in enumerate(title.split()):
            r = search(title.rsplit(' ', i)[0], year, False)
            if r is not None: return r
            checklist.append(r)
        return next((t for t in checklist if t is not None), None)

    # ... omitting 'year'
    elif len(results) == 0 and year is not None:
        o.debug('0 results - omit year and try again')
        return search(title, None, recur)    
    ...
    # Yeah! There are some results. Let's check them to see if they're a match
        ...
        for result in results:
            ...
            if checkMatch(title, year, proposedTitle, proposedYear, popularity):
                return {
                    "title": proposedTitle,
                    "year": proposedYear,
                    "id": id
                ...
                if year is not None: return search(title, None)

def checkMatch(title, year, proposedTitle, proposedYear, popularity):
    ...
    o.debug("Comparing match: {}=={}, {}=={} (year diff: {}, popularity: {}, title similarity: {})".format(origTitle, proposedTitle, year, proposedYear, yearSimilarity, popularity, titleSimilarity))
...
def yearsDeviation(year, proposedYear):
    ...
    if year is None or proposedYear is None: return 0
